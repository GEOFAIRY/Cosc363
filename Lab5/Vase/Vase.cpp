...
void display(void)
{
	...
	for (int j = 0; j < 36; j++) {
		...
		for (int i = 0; i < N; i++)
		{
			...
			glTexCoord2f(static_cast<float>(j) / 36.0, static_cast<float>(i) / N); glVertex3f(vx[i], vy[i], vz[i]);
			if (i > 0) normal(wx[i - 1], wy[i - 1], wz[i - 1],vx[i], vy[i], vz[i], wx[i], wy[i], wz[i]);
			glTexCoord2f(static_cast<float>(1 + j) / 36.0, static_cast<float>(i) / N); glVertex3f(wx[i], wy[i], wz[i]);
		}
		glEnd();
