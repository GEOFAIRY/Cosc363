...
glm::vec3 trace(Ray ray, int step)
{
	...
	//create the ray color
	glm::vec3 colSum;
	if (lDotn < 0) {
		colSum = (ambientCol * materialCol + specularRef);
	}
	else {
		colSum = ((ambientCol * materialCol) + (lDotn * materialCol)) + specularRef;
	}
	...
	//Missing texture hah
	if (ray.xindex == 3) {
		int xmod = int((ray.xpt.x + 25) / 10) % 2;
		int zmod = int((ray.xpt.z + 25) / 10) % 2;

		if ((xmod && zmod) || (!xmod && !zmod)) {
			sceneObjects[3]->setColor(glm::vec3(0, 0, 0));
		} else {
			sceneObjects[3]->setColor(glm::vec3(1, 0, 0.75));
		}
	}

	//reflection for sphere 0
	if (ray.xindex == 0 && step < MAX_STEPS)
	{
		glm::vec3 reflectedDir = glm::reflect(ray.dir, normalVector);
		Ray reflectedRay(ray.xpt, reflectedDir);
		glm::vec3 reflectedCol = trace(reflectedRay, step + 1); //Recursion!
		colSum = colSum + (0.8f * reflectedCol);
	}
...
void display()
{
	...
	for(int i = 0; i < NUMDIV; i++)  	//For each grid point xp, yp
	{
		...
		for(int j = 0; j < NUMDIV; j++)
		{
			...
			float col_total_r = (col1.r + col2.r + col3.r + col4.r)/4;
			float col_total_g = (col1.g + col2.g + col3.g + col4.g)/4;
			float col_total_b = (col1.b + col2.b + col3.b + col4.b)/4;
			glColor3f(col_total_r, col_total_g, col_total_b);
...
void initialize()
{
	...
	//-- Create a pointer to a sphere object
	Sphere* sphere2 = new Sphere(glm::vec3(4.0, 4.0, -60.0), 2.0, glm::vec3(1, 0, 0));

	//--Add the above to the list of scene objects.
	sceneObjects.push_back(sphere2);

	//-- Create a pointer to a sphere object
	Sphere* sphere3 = new Sphere(glm::vec3(11.0, 8.0, -70.0), 4.0, glm::vec3(0, 1, 0));

	//--Add the above to the list of scene objects.
	sceneObjects.push_back(sphere3);

	//-- Create a pointer to a new plane object
