...
bool Plane::isInside(glm::vec3 pt)
{
	...
	float insideA = glm::dot(cross((b-a), (pt-a)), n);
	float insideB = glm::dot(cross((c-b), (pt-b)), n);
	float insideC = glm::dot(cross((d-c), (pt-c)), n);
	float insideD = glm::dot(cross((a-d), (pt-d)), n);
